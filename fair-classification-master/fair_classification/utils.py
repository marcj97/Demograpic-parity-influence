...
def get_dem_par_acc(model, x_train, y_train, x_control_train, y_train_predicted):


    """
    returns the demograpic parity score of the model
    we either pass the model (w)
    else we pass y_predicted
    """
    if model is not None and y_train_predicted is not None:
        print("Either the model (w) or the predicted labels should be None")
        raise Exception("Either the model (w) or the predicted labels should be None")

    if model is not None:
        y_train_predicted = np.sign(np.dot(x_train, model))
    def with_pred_count(y, Y_predicted, x_control, group_binary, pred_value):
        if not(len(y) == len(Y_predicted) and len(y) == len(x_control["sex"])):
            print(len(y), " ; ", len(Y_predicted)," ; ",  len(x_control["sex"]))
            raise Exception("length not the same")
            
        count = 0 
        for i in range(0,len(y)):
            if(x_control["sex"][i] == group_binary and Y_predicted[i] == pred_value):
                count += 1
        return count

    def without_pred_count(y, Y_predicted, x_control, group_binary):
        if not(len(y) == len(Y_predicted) and len(y) == len(x_control["sex"])):
            print(len(y), " ; ", len(Y_predicted)," ; ",  len(x_control["sex"]))
            raise Exception("length not the same")
        ...
        count = 0 
        for i in range(0,len(y)):
            if(x_control["sex"][i] == group_binary):
                count += 1
        return count
    
    pred_count = with_pred_count(y_train, y_train_predicted, x_control_train, 0, 1)
    non_pred_count = without_pred_count(y_train, y_train_predicted, x_control_train, 0)
    chance_pred_one_bin_zero = pred_count/non_pred_count

    pred_count = with_pred_count(y_train, y_train_predicted, x_control_train, 1, 1)
    non_pred_count = without_pred_count(y_train, y_train_predicted, x_control_train, 1)
    chance_pred_one_bin_one = pred_count/non_pred_count

    pred_count = with_pred_count(y_train, y_train_predicted, x_control_train, 0, -1)
    non_pred_count = without_pred_count(y_train, y_train_predicted, x_control_train, 0)
    chance_pred_zero_bin_zero = pred_count/non_pred_count

    pred_count = with_pred_count(y_train, y_train_predicted, x_control_train, 1, -1)
    non_pred_count = without_pred_count(y_train, y_train_predicted, x_control_train, 1)
    chance_pred_zero_bin_one = pred_count/non_pred_count

    score = min(chance_pred_one_bin_one/chance_pred_one_bin_zero,chance_pred_one_bin_zero/chance_pred_one_bin_one,chance_pred_zero_bin_zero/chance_pred_zero_bin_one,chance_pred_zero_bin_one/chance_pred_zero_bin_zero)
    print("demograpic parity score:", score)
    return score

def get_eq_op_acc(model, x_train, y_train, x_control_train, y_train_predicted):


    """
    returns the equality of opportunity score of the model
    we either pass the model (w)
    else we pass y_predicted
    """
    if model is not None and y_train_predicted is not None:
    ...
    if model is not None:
        y_train_predicted = np.sign(np.dot(x_train, model))
    def with_pred_count(y, Y_predicted, x_control, group_binary):
        if not(len(y) == len(Y_predicted) and len(y) == len(x_control["sex"])):
            print(len(y), " ; ", len(Y_predicted)," ; ",  len(x_control["sex"]))
            raise Exception("length not the same")
        ...
        count = 0 
        for i in range(0,len(y)):
            if(x_control["sex"][i] == group_binary and y[i] == 1 and Y_predicted[i] == 1):
                count += 1
        return count
    ...
    def without_pred_count(y, Y_predicted, x_control, group_binary):
        ...
        count = 0 
        for i in range(0,len(y)):
            if(x_control["sex"][i] == group_binary and y[i] == 1):
                count += 1
        return count
    
    pred_count = with_pred_count(y_train, y_train_predicted, x_control_train, 0)
    non_pred_count = without_pred_count(y_train, y_train_predicted, x_control_train, 0)
    chance_bin_zero = pred_count/non_pred_count
    
    pred_count = with_pred_count(y_train, y_train_predicted, x_control_train, 1)
    non_pred_count = without_pred_count(y_train, y_train_predicted, x_control_train, 1)
    chance_bin_one = pred_count/non_pred_count
    score = min(chance_bin_one/chance_bin_zero,chance_bin_zero/chance_bin_one)
    print("equality of opportunity score:", score)
    return score, chance_bin_zero, chance_bin_one

def get_eq_odds_acc(model, x_train, y_train, x_control, y_train_predicted):


    """
    returns the equalized odds score of the model
    we either pass the model (w)
    else we pass y_predicted
    """
    if model is not None and y_train_predicted is not None:
    ...
    if not(len(y_train) == len(y_train_predicted) and len(y_train) == len(x_control["sex"])):
        print(len(y_train), " ; ", len(y_train_predicted)," ; ",  len(x_control["sex"]))
        raise Exception("length not the same")
    ...
    score, chance_attr_zero_pred_one, chance_attr_one_pred_one = get_eq_op_acc(None, x_train, y_train, x_control, y_train_predicted)
    
    pred_count = with_pred_count(y_train, y_train_predicted, x_control, 0)
    non_pred_count = without_pred_count(y_train, y_train_predicted, x_control, 0)
    chance_attr_zero_pred_zero = (pred_count/non_pred_count)

    pred_count = with_pred_count(y_train, y_train_predicted, x_control, 1)
    non_pred_count = without_pred_count(y_train, y_train_predicted, x_control, 1)
    chance_attr_one_pred_zero = (pred_count/non_pred_count)
    
    score = min((chance_attr_zero_pred_one/chance_attr_one_pred_one),(chance_attr_one_pred_one/chance_attr_zero_pred_one),(chance_attr_zero_pred_zero/chance_attr_one_pred_zero),(chance_attr_one_pred_zero/chance_attr_zero_pred_zero))
    print("equalized odds score:", score)
    return score

def get_pred_rate_par_acc(model, x_train, y_train, x_control, y_train_predicted):


    """
    returns the predictive rate parity score of the model
    we either pass the model (w)
    else we pass y_predicted
    """
    if model is not None and y_train_predicted is not None:
    ...
    def with_actual_count(y, Y_predicted, x_control, y_binary, pred_binary, control_binary):
        count = 0 
        for i in range(0,len(y)):
            if(x_control["sex"][i] == control_binary and y[i] == y_binary and Y_predicted[i] == pred_binary):
                count += 1
        return count

    def without_actual_count(Y_predicted, x_control, pred_binary, control_binary):
        count = 0 
        for i in range(0,len(Y_predicted)):
            if(x_control["sex"][i] == control_binary and Y_predicted[i] == pred_binary):
                count += 1
        return count
    
    actual_count = with_actual_count(y_train, y_train_predicted, x_control, 1, 1, 0)
    non_actual_count = without_actual_count(y_train_predicted, x_control, 1, 0)
    chance_attr_zero_one = (actual_count/non_actual_count)

    actual_count = with_actual_count(y_train, y_train_predicted, x_control, 1, 1, 1)
    non_actual_count = without_actual_count(y_train_predicted, x_control, 1, 1)
    chance_attr_one_one = (actual_count/non_actual_count)
    
    actual_count = with_actual_count(y_train, y_train_predicted, x_control, -1, -1, 0)
    non_actual_count = without_actual_count(y_train_predicted, x_control, -1, 0)
    chance_attr_zero_zero = (actual_count/non_actual_count)

    actual_count = with_actual_count(y_train, y_train_predicted, x_control, -1, -1, 1)
    non_actual_count = without_actual_count(y_train_predicted, x_control, -1, 1)
    chance_attr_one_zero = (actual_count/non_actual_count)
    
    score = min((chance_attr_zero_one/chance_attr_one_one),(chance_attr_one_one/chance_attr_zero_one),(chance_attr_zero_zero/chance_attr_one_zero),(chance_attr_one_zero/chance_attr_zero_zero))
    print("Predictive Rate Parity score:", score)
    return score
